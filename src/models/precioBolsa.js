...
const PrecioBolsa = {
  getAll(page = 1, limit = 10, callback) {
    ...
    db.query("SELECT COUNT(*) AS total FROM precio_bolsa", (err, countResult) => {
      if (err) {
      ...
      db.query(
        "SELECT * FROM precio_bolsa ORDER BY id ASC LIMIT ? OFFSET ?",
  ...
  create: (data, callback) => {
    ...
    db.query(
      "INSERT INTO precio_bolsa (fecha, titulo, descripcion, documento_pdf) VALUES (?, ?, ?, ?)",
  ...
  update: (id, data, callback) => {
    const { fecha, titulo, descripcion, documento_pdf } = data;
    db.query(
      "UPDATE precio_bolsa SET fecha = ?, titulo = ?, descripcion = ?, documento_pdf = ? WHERE id = ?",
  ...
  delete: (id, callback) => {
    db.query("DELETE FROM precio_bolsa WHERE id = ?", [id], callback);
  },
};

module.exports = PrecioBolsa;
